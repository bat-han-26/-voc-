...
CONCEPTS = {
    ...
    "발색": ["발색"], "지속력/색빠짐": ["지속", "오래", "안지워", "색이 남", "색빠", "유지", "무너지"],
    "밀착": ["밀착"], "가루날림": ["가루", "날림", "날려", "흩날"],
    "촉촉/보습": ["촉촉", "보습", "수분"], "건조/각질/들뜸": ["건조", "각질", "갈라", "들뜨", "들뜸"],
    "커버력/모공": ["커버", "모공", "잡티", "결점"], "자연스러움": ["자연스", "은은", "과하지"],
    "광택/윤기": ["광택", "윤기", "글로시", "물광"], "매트/보송": ["매트", "보송"],
    "부드러운 발림": ["부드럽", "스무스", "얇게"], "데일리/일상": ["데일리", "매일", "일상", "평소"],
    "직장/오피스/행사": ["직장", "회사", "오피스", "출근", "성인식", "면접", "학교"],
    "선물/기프트": ["선물", "기프트", "증정용"], "가성비/가격": ["가성비", "저렴", "가격", "합리적"],
    "용량/사이즈": ["용량", "사이즈", "미니", "작아", "크기"],
    "콜라보/캐릭터": ["키티", "헬로키티", "짱구", "콜라보", "캐릭터", "산리오", "잔망", "파워퍼프"],
    "패키지/디자인": ["패키지", "케이스", "디자인", "파우치", "비주얼"],
    "사은품/굿즈": ["사은품", "키링", "굿즈", "증정품", "특전"],
    "배송/운영": ["배송", "느리", "느려", "늦", "품절", "재입고", "재고"],
    "재구매/충성": ["재구매", "또 살", "또 구매", "리피", "다시 살", "쟁여"],
    "프로모션/할인": ["할인", "메가할", "한정", "세일", "쿠폰", "기획전"],
}
...
def agg_group(g):
    ...
    # ---- VOC 니즈(개념) 빈도 + 실제 표현(bigram) ----
    kr = trans["review_content_kr"].astype(str)
    tn = max(1, len(kr))
    needs = []
    for c, kws in CONCEPTS.items():
        hit = int(kr.apply(lambda t: any(k in t for k in kws)).sum())
        if hit:
            needs.append({"need": c, "count": hit, "pct": round(hit / tn * 100, 1)})
...
def main():
    ...
    STRATEGY = {
        "1040221567": {  # 소블아 (아이섀도우)
            "conclusion": [
                "구매·만족의 1차 결정요인은 <b>'내 퍼스널컬러에 맞는 발색'</b>(발색 15%·쿨/웜톤 언급 10%). 단일 색이 아닌 <b>'버릴 색 하나 없는' 팔레트 활용도</b>가 핵심 만족 포인트.",
                "유일하게 반복되는 품질 리스크는 <b>가루 날림</b>(저평점 변별 키워드). 그러나 저평점의 상당수는 제품이 아니라 <b>사은품(키링) 누락·재고</b> 등 운영 이슈 → 제품력보다 CX로 방어 가능.",
            ],
            "ugc": [
                "<b>퍼스널컬러(쿨/웜)별 전색 스와치 + 데일리 그라데이션</b> 튜토리얼 — '버릴 색 없다'는 실사용 언어를 그대로 후킹 카피로.",
                "<b>'가루 날림 잡는 법'</b>(아이섀도 베이스 병행) 솔직 팁 콘텐츠로 약점을 선제 커버 → 신뢰형 마이크로 인플루언서에 최적.",
                "미니 사이즈·가성비·콜라보 패키지 <b>언박싱/데스크테리어</b>로 소장·기프트 수요 자극.",
            ],
            "language": ["버릴 색 하나 없이", "사용하기 편한 색", "발색도 좋고", "데일리로 좋아요", "가루 날림 없어요", "쿨톤/웜톤에 찰떡"],
        },
    ...
    overall = [
        ...
        "<b>카테고리별 니즈 분리</b>: 아이섀도우=발색·전색활용·가루날림, 파운데이션=커버/모공·피부타입 적합성, 틴트=물광·색지속·향. 콘텐츠 메시지를 카테고리별로 다르게 설계.",
        "<b>UGC 핵심 원칙</b>: 소비자 실제 언어(아래 '실제 언어')를 카피로 그대로 차용하고, 각 제품의 알려진 약점(가루날림/건조/배송)을 <b>솔직하게 선제 언급</b>하는 마이크로 인플루언서가 신뢰·전환에 유리.",
    ]
